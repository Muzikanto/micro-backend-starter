...
export const TransformArray =
  // eslint-disable-next-line
    ...
    <A = string, R = string>(
      Convert: new (v: A) => R,
      { nullable, example }: { nullable?: boolean; example?: any } = {}
    ): PropertyDecorator =>
    (target: object, propertyKey: string | symbol) => {
      Transform((params) => params.value?.map((el: A) => new Convert(el)));
      IsArray({ message: DtoMessages.isArray })(target, propertyKey);
      Type(() => Convert)(target, propertyKey);

      if (!nullable) {
        ApiProperty({ nullable, type: Convert, isArray: true, example })(target, propertyKey);
        IsNotEmpty({ message: 'DtoMessages.isNotEmpty' })(target, propertyKey);
      } else {
        ApiPropertyOptional({ nullable, type: Convert, isArray: true, example })(target, propertyKey);
        IsOptional()(target, propertyKey);
      }
...
export const TransformObject =
  // eslint-disable-next-line
    ...
    <A = string, R = string>(Convert: new (v: A) => R, { nullable }: { nullable?: boolean } = {}): PropertyDecorator =>
    (target: object, propertyKey: string | symbol) => {
      ...
      if (!nullable) {
        ApiProperty({ nullable, type: Convert })(target, propertyKey);
        IsNotEmpty({ message: DtoMessages.isNotEmpty })(target, propertyKey);
      } else {
        ApiPropertyOptional({ nullable, type: Convert })(target, propertyKey);
        IsOptional()(target, propertyKey);
      }
